---
title: "ML1"
author: "Alvaro Cervan, Luca Renz, Rafaella Miranda-Sousa"
date: "`r Sys.Date()`"
output:
...
In order to apply such algorithms, the data has to be preprocessed. 
...
```{r data_prep, echo=FALSE, cache=TRUE, results='hide'}
...
```{r graph_data_analysis, cache = TRUE, fig.width=12, fig.height=6}
...
clean_dat_motor$AMOUNT_CLAIMS_PAID_log <- log1p(clean_dat_motor$AMOUNT_CLAIMS_PAID)
clean_dat_motor$SEATS_NUM_log <- log1p(clean_dat_motor$SEATS_NUM)

# Set up layout for 2 plots per row
par(mfrow = c(2, 2), oma = c(0, 0, 2, 0), mar = c(4, 4, 2, 1)) 

# Histograms
hist(clean_dat_motor$PREMIUM, main = "Hist. of PREMIUM", xlab = "PREMIUM", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$PREMIUM_log, main = "Hist. of PREMIUM_log", xlab = "PREMIUM_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

hist(clean_dat_motor$INSURED_VALUE, main = "Hist. of INSURED_VALUE", xlab = "INSURED_VALUE", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$INSURED_VALUE_log, main = "Hist. of INSURED_VALUE_log", xlab = "INSURED_VALUE_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

hist(clean_dat_motor$CLAIM_PAID_USD, main = "Hist. of CLAIM_PAID_USD", xlab = "CLAIM_PAID_USD", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$CLAIM_PAID_USD_log, main = "Hist. of CLAIM_PAID_USD_log", xlab = "CLAIM_PAID_USD_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

hist(clean_dat_motor$AMOUNT_CLAIMS_PAID, main = "Hist. of AMOUNT_CLAIMS_PAID", xlab = "AMOUNT_CLAIMS_PAID", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$AMOUNT_CLAIMS_PAID_log, main = "Hist. of AMOUNT_CLAIMS_PAID_log", xlab = "AMOUNT_CLAIMS_PAID_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

hist(clean_dat_motor$CCM_TON, main = "Hist. of CCM_TON", xlab = "CCM_TON", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$CCM_TON_log, main = "Hist. of CCM_TON_log", xlab = "CCM_TON_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

hist(clean_dat_motor$SEATS_NUM, main = "Hist. of SEATS_NUM", xlab = "SEATS_NUM", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
hist(clean_dat_motor$SEATS_NUM_log, main = "Hist. of SEATS_NUM_log", xlab = "SEATS_NUM_log", col = "skyblue", border = "black", breaks = 30, freq = TRUE)

par(mfrow = c(2, 1))
hist(clean_dat_motor$AGE_VEHICLE, main = "Hist. of AGE_VEHICLE", xlab = "AGE_VEHICLE", col = "skyblue", border = "black", breaks = 30, freq = TRUE)
...
boxplot(PREMIUM ~ INSR_TYPE, data = clean_dat_motor,
        main = "Premium against INSR_TYPE",
        ylab = "Premium")

boxplot(PREMIUM ~ USAGE, data = clean_dat_motor,
        main = "Premium against USAGE",
        ylab = "Premium")

boxplot(PREMIUM ~ TYPE_VEHICLE, data = clean_dat_motor,
        main = "Premium against TYPE_VEHICLE",
        ylab = "Premium")

par(mfrow = c(1, 1))
boxplot(PREMIUM ~ MAKE, data = clean_dat_motor,
        main = "Premium against MAKE",
        ylab = "Premium")

# Set up layout for 2 plots per row
par(mfrow = c(2, 2), oma = c(0, 0, 2, 0), mar = c(4, 4, 2, 1)) 

ggplot(data = clean_dat_motor,
mapping = aes(y = PREMIUM,
x = CLAIM_PAID_USD)) +
geom_point() +
...
#geom_point() +
#geom_smooth(se = FALSE) +
#facet_wrap(. ~ INSR_TYPE) ## ! new argument
#
#ggplot(data = clean_dat_motor,
#mapping = aes(y = PREMIUM,
#x = INSURED_VALUE)) +
...
#geom_point() +
#geom_smooth(se = FALSE) +
#facet_wrap(. ~ TYPE_VEHICLE) ## ! new argument
#
#ggplot(data = clean_dat_motor,
#mapping = aes(y = PREMIUM,
#x = INSURED_VALUE)) +
...
The relationships between PREMIUM and all numerical predictors except INSURED_VALUE appear to be linear and are included in the models as linear effects. The effect of INSURED_VALUE is clearly non-linear. The smoothing indicates that we could model this relationship as a quadratic effect.However, the effect is smoothed by logarithmising INSURED_VALUE.
...
```{r linear_model, cache = TRUE, results='hide'}
#Fit the linear model
lm_model_0 <- lm(PREMIUM_log ~ SEX + INSR_TYPE + USAGE + TYPE_VEHICLE + MAKE +
                 AGE_VEHICLE + SEATS_NUM_log + CCM_TON_log + INSURED_VALUE_log +
                 AMOUNT_CLAIMS_PAID_log, data = clean_dat_motor)

#Perform an F-test to drop non-significant categorical variables one at a time
#drop1(lm_model, test= "F") 

#Fit the new linear model
lm_model <- lm(PREMIUM_log ~ SEX + INSR_TYPE + USAGE + TYPE_VEHICLE + MAKE +
                 AGE_VEHICLE + SEATS_NUM_log + INSURED_VALUE_log +
                 AMOUNT_CLAIMS_PAID_log, data = clean_dat_motor)
...
p_value <- "< 2.2e-16"  # p-value is very small and can be directly set as this value
adjusted_r_squared <- round(model_summary$adj.r.squared, 4)  # Adjusted R-squared

# Create a data frame for the table
result_table <- data.frame(
  Statistic = c("p-value", "Adjusted R-squared", "Mean Squared Error"),
  Value = c(p_value, adjusted_r_squared, round(mse, 4))  # Rounded values for better readability
)

# Print the table
knitr::kable(result_table, caption = "Results of the Linear Regression Model")
...
The model summary shows that the Multiple R-squared value is 0.7307, indicating that the model can explain approximately 73.07% of the variance in premiums. This suggests that the model provides a good fit to the data. The F-test for the overall model is significant (p < 2.2e-16), indicating that the predictors as a group have a substantial effect on the premium. All predictors have a significant impact on the target variable PREMIUM_log. The Modell explains 73.06% of the variance in PREMIUM_log (Adjusted R-squared: 0.7306). The Mean Squared Error (MSE) of 0.2536 indicates a good fit with relatively low average prediction errors.
...
INSURED_VALUE_log has the strongest impact with a coefficient of 0.762, meaning that a 1% increase in the insured value leads to an approximately 76.2% increase in the logarithmic premium. Similarly, AMOUNT_CLAIMS_PAID_log (0.208) indicates that a 1% increase in claims paid corresponds to an approximately 20.8% increase in the logarithmic premium, and SEATS_NUM_log (0.016) suggests a 1% increase in the number of seats results in a 1.6% increase in the logarithmic premium. Meanwhile, AGE_VEHICLE shows a smaller but still significant effect with a coefficient of 0.0026, where each additional year increases the logarithmic premium by approximately 0.26%.

VIF:
An analysis of multicollinearity revealed that the Variance Inflation Factor (VIF) for the variable INSR_TYPE is 5.88, which suggests possible multicollinearity. This could affect the model’s stability and interpretability and should be considered in further model optimization.
...
```{r data_for_poisson, cache=TRUE, results='hide'}
...
hist(
  dat_amount_claims$AMOUNT_CLAIMS,
  breaks = max(dat_amount_claims$AMOUNT_CLAIMS),  # Number of bins equal to the max value of AMOUNT_CLAIMS
  col = "skyblue",  # Bar color
  border = "black",  # Border color for bars
  main = "Distribution of AMOUNT_CLAIMS",  # Plot title
  xlab = "Number of Claims",  # Label for the x-axis
  ylab = "Frequency",  # Label for the y-axis
  las = 1,  # Make y-axis labels horizontal
  cex.main = 1.2,  # Increase font size for the title
  cex.lab = 1.1,  # Increase font size for axis labels
  cex.axis = 1  # Increase font size for axis values
)

# Add grid lines for better readability
grid(nx = NULL, ny = NULL, col = "gray", lty = "dotted")  # Add gray dotted grid lines
#-> Zero-inflated Poisson Regression would fit better

#Mean vs. Variance
mean_amount_claims <- mean(dat_amount_claims$AMOUNT_CLAIMS, na.rm = TRUE)
var_amount_claims <- var(dat_amount_claims$AMOUNT_CLAIMS, na.rm = TRUE)

table <- data.frame(
  Statistic = c("Mean", "Variance"),
  AMOUNT_CLAIMS = c(mean_amount_claims, var_amount_claims)
)

print(table) #The variance is not much greater than the mean, over-dispersion can be neglegted in the model.
...
```{r Poisson_Model, cache=TRUE, results='hide'}
...
```{r Binomial_Model, cache=TRUE, results='hide'}
#Fitting a binomial regression model
fit.binom <- glm(CLAIM_PAID ~ SEX + INSR_TYPE +  USAGE + TYPE_VEHICLE +
                   MAKE + AGE_VEHICLE + SEATS_NUM_log + CCM_TON_log + INSURED_VALUE_log +
                   PREMIUM_log + AMOUNT_CLAIMS_PAID_log,
...
fit.binom.sign <- glm(
  CLAIM_PAID ~ SEX + USAGE + TYPE_VEHICLE + MAKE + AGE_VEHICLE + SEATS_NUM_log +
               CCM_TON_log + INSURED_VALUE_log + PREMIUM_log + AMOUNT_CLAIMS_PAID_log,
  data = clean_dat_motor,
  family = binomial(link = "logit")
)
...
# Global F-test calculation for the new model
...
p_value_no_insr_seats <- 1 - pchisq(TD_no_insr_seats, df = df_diff_no_insr_seats)  # p-value

# Calculate AIC
aic_value <- AIC(fit.binom.sign)

# Pseudo-R² (Nagelkerke)
null_model <- glm(CLAIM_PAID ~ 1, data = clean_dat_motor, family = binomial(link = "logit"))
pseudo_r2 <- 1 - (logLik(fit.binom.sign) / logLik(null_model))

# Overdispersion ratio
overdispersion_ratio <- summary(fit.binom.sign)$deviance / summary(fit.binom.sign)$df.residual

# Likelihood Ratio Test statistic against the null model
lrt_statistic <- -2 * (logLik(null_model) - logLik(fit.binom.sign))

# AUC
auc_value <- auc(
  response = as.numeric(clean_dat_motor$CLAIM_PAID),  # Sicherstellen, dass binär und numerisch
  predictor = predict(fit.binom.sign, type = "response")
)

# Combine results into a table
result_table <- data.frame(
  Statistic = c(
    "Global test statistic (TD)", 
    "p-value of global test", 
    "AIC", 
  ...
  Value = c(
    round(TD_no_insr_seats, 4),
    format.pval(p_value_no_insr_seats, digits = 4, scientific = TRUE),
    round(aic_value, 2),
    round(pseudo_r2, 4),
    round(overdispersion_ratio, 4),
    round(lrt_statistic, 4),
    round(auc_value, 4)
  )
)

# Print the table
knitr::kable(result_table, caption = "Results of the Binomial Regression Model")
...
For AGE_VEHICLE, is steady until 40 years where it declines rapidly, indicating that older vehicles have fewer claims. 
...
# plotnet(nn_model, alpha = 0.5, margin = 0.1, cex = 0.8) ### Too many inputs to plot
...
```{r SVM MCC, cache=TRUE, results='hide'}
...
print("Confusion metrics for TRAIN_DATA")
...
pred_test <- predict(svm_model, newdata = test_data)
conf_matrix_test <- confusionMatrix(pred_test, test_data$premium_category)
print("Confusion metrics for TEST_DATA")
print(conf_matrix_test)

# Manual MCC Calculation for Test Data
conf_table_test <- as.table(conf_matrix_test$table)
TP_test <- conf_table_test[2, 2]  # True Positive
TN_test <- conf_table_test[1, 1]  # True Negative
FP_test <- conf_table_test[1, 2]  # False Positive
FN_test <- conf_table_test[2, 1]  # False Negative
mcc_test_manual <- (TP_test * TN_test - FP_test * FN_test) / sqrt((TP_test + FP_test) * (TP_test + FN_test) * (TN_test + FP_test) * (TN_test + FN_test))

print(paste("MCC manually calculated for Test Data:", round(mcc_test_manual, 4)))
