...
## GAM v3 - Predicting Premium Amount
```{r GAM v3}
# Fit a Generalized Additive Model (GAM) to the data to predict `PREMIUM`

# Check dataset structure and set up training/testing data
set.seed(123)  # For reproducibility
train_indices <- sample(seq_len(nrow(dat_amount_claims)), size = 0.7 * nrow(dat_amount_claims))
train_data <- dat_amount_claims[train_indices, ]
test_data <- dat_amount_claims[-train_indices, ]

# Fit a GAM model to predict `PREMIUM`
gam_premium <- gam(
  PREMIUM ~ s(INSURED_VALUE) + s(AGE_VEHICLE) + s(SEATS_NUM) + 
    s(CCM_TON) + INSR_TYPE + USAGE + TYPE_VEHICLE,
  family = Gamma(link = "log"),  # Suitable for continuous, positive, skewed data
  data = train_data
)
summary(gam_premium)
# Model evaluation
# Predict on test set for PREMIUM
test_data$premium_predictions <- predict(gam_premium, newdata = test_data, type = "response")

# Calculate RMSE for PREMIUM predictions
premium_rmse <- sqrt(mean((test_data$PREMIUM - test_data$premium_predictions)^2))
print(paste("Premium RMSE:", premium_rmse))

# Visualize actual vs predicted PREMIUM
ggplot(test_data, aes(x = PREMIUM, y = premium_predictions)) +
  geom_point(alpha = 0.5, color = "darkblue") +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "Actual vs Predicted PREMIUM", x = "Actual PREMIUM", y = "Predicted PREMIUM") +
  theme_minimal()

# RMSE
rmse_gam_premium <- sqrt(mean((test_data$PREMIUM - test_data$premium_predictions)^2))
```

## GAM v4 - Predicting Claims Amount
```{r GAM v4}
# Fit a GAM model to predict `AMOUNT_CLAIMS_PAID`
gam_claims <- gam(
  AMOUNT_CLAIMS ~ s(INSURED_VALUE) + s(AGE_VEHICLE) + s(SEATS_NUM) + 
    s(CCM_TON) + INSR_TYPE + USAGE + TYPE_VEHICLE,
...
# Model evaluation
# Predict on test set for CLAIMS
test_data$claims_predictions <- predict(gam_claims, newdata = test_data, type = "response")

# Calculate RMSE for CLAIM predictions
claims_rmse <- sqrt(mean((test_data$AMOUNT_CLAIMS - test_data$claims_predictions)^2))
print(paste("Claims RMSE:", claims_rmse))

# Visualize actual vs predicted CLAIMS
ggplot(test_data, aes(x = AMOUNT_CLAIMS, y = claims_predictions)) +
  geom_point(alpha = 0.5, color = "darkgreen") +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "Actual vs Predicted CLAIMS", x = "Actual AMOUNT_CLAIMS_PAID", y = "Predicted AMOUNT_CLAIMS_PAID") +
  theme_minimal()


# RMSE 
rmse_gam_claims <- sqrt(mean((test_data$AMOUNT_CLAIMS - test_data$claims_predictions)^2))
cat("RMSE of the GAM model for CLAIMS:", rmse_gam_claims, "\n")

```

## GAM v5 - Predicting Number of Claims

```{r Gam v5}
#A GAM model is fitted to predict the number of claims over a 5-year period based on the characteristics SEX, INSR_TYPE, USAGE, TYPE_VEHICLE, MAKE, AGE_VEHICLE, SEATS_NUM, CCM_TON, INSURED_VALUE, and PREMIUM.
...
# Fit a GAM model to predict the number of claims
gam_model <- mgcv::gam(
  AMOUNT_CLAIMS ~ SEX + INSR_TYPE + USAGE + TYPE_VEHICLE + MAKE +
    s(AGE_VEHICLE, bs = "cr") +
    s(SEATS_NUM, bs = "cr") +
...
summary(gam_model)
# Predict on the data for visualization
dat_amount_claims$predicted_claims <- predict(gam_model, newdata = dat_amount_claims, type = "response")
