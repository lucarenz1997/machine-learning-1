...
```{r SVM_Rafi_turbo, cache=TRUE}


# Parallelisierung einrichten
cl <- makeCluster(detectCores() - 1) # Verwende alle verfügbaren Kerne außer einem
registerDoParallel(cl)

# Datenvorbereitung und Stichprobenerstellung
set.seed(123)
clean_dat_motor2 <- sample_n(clean_dat_motor, size = 1000)

# Kontrollparameter für wiederholte Kreuzvalidierung
ctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)

# Hyperparameter-Raster für Tuning
tune_grid <- expand.grid(
  .C = c(0.1, 1, 10),        # Reduzierte Werte für 'cost'
  .sigma = c(0.01, 0.1)      # Reduzierte Werte für 'gamma'
)

# Aufteilen der Daten in Training und Test (z. B. 70% Training, 30% Test)
set.seed(123)
sample_index <- sample(seq_len(nrow(clean_dat_motor2)), size = 0.7 * nrow(clean_dat_motor2))
train_data <- clean_dat_motor2[sample_index, ]
test_data <- clean_dat_motor2[-sample_index, ]

# SVM-Modelltraining mit Kreuzvalidierung und Hyperparameter-Tuning
svm_model <- train(CLAIM_PAID ~ SEX + INSR_TYPE + USAGE + TYPE_VEHICLE + MAKE + AGE_VEHICLE +
                     SEATS_NUM + CCM_TON_log + INSURED_VALUE_log + PREMIUM_log + AMOUNT_CLAIMS_PAID,
                   data = train_data,
                   method = "svmRadial",
                   trControl = ctrl,
                   tuneGrid = tune_grid)

# Stoppe den Cluster nach der Berechnung
stopCluster(cl)

# Modellbewertung
print(svm_model$bestTune)     # Zeigt die besten Hyperparameter
print(svm_model$finalModel)   # Zeigt das endgültige Modell
...
# Konfusionsmatrix zur Bewertung der Modellleistung
confusion_matrix <- table(Predicted = predictions, Actual = test_data$CLAIM_PAID)
...
print(confusionMatrix(data = pred_train, reference = train_data$CLAIM_PAID))

# Genauigkeit der Testdaten
pred_test <- predict(svm_model, test_data)
print(confusionMatrix(data = pred_test, reference = test_data$CLAIM_PAID))


```


```{r SVM_Rafi, cache=TRUE}
...
sample_index <- sample(seq_len(nrow(clean_dat_motor)), size = 0.7 * nrow(clean_dat_motor))
train_data <- clean_dat_motor[sample_index, ]
test_data <- clean_dat_motor[-sample_index, ]

# Train the SVM model with cross-validation and hyperparameter tuning
svm_model <- train(CLAIM_PAID ~ SEX + INSR_TYPE + USAGE + TYPE_VEHICLE + MAKE + AGE_VEHICLE +
                     SEATS_NUM + CCM_TON_log + INSURED_VALUE_log + PREMIUM_log + AMOUNT_CLAIMS_PAID,
                   data = train_data,
                   method = "svmRadial",     # SVM mit radialem Kernel
                   trControl = ctrl,          # Kontrollparameter für Kreuzvalidierung
                   tuneGrid = tune_grid)      # Hyperparameter-Raster für Tuning
...
#Aus den 857 Trainingsdaten wurden 469 Support vectors kreiert (Siehe Train Anzl DS & Output)
...
#Accuracy der Trainigsdaten
pred_train<- predict(svm_model, train_data)
confusionMatrix(data= pred_train , reference= train_data$CLAIM_PAID)
#Accuracy der Testdaten
pres_test<- predict(svm_model, test_data)
confusionMatrix(data= pres_test , reference= test_data$CLAIM_PAID)
...
```{r SVM preparation, cache=TRUE}
set.seed(123)
svm_data <- clean_dat_motor[sample(nrow(clean_dat_motor), 10000, replace = FALSE), ]
...
selected_data <- svm_data[, c("AGE_VEHICLE", "INSURED_VALUE_log", "premium_category")]
...
# Hyperparameter-Raster für Tuning
tune_grid <- expand.grid(
  .C = c(0.1, 1, 10),       # Werte für 'cost'
  .sigma = c(0.01, 0.1, 0.5) # Werte für 'gamma'
)

# Set seed for reproducibility and create a subset of the data
set.seed(123)
svm_data <- clean_dat_motor[sample(nrow(clean_dat_motor), 1000, replace = FALSE), ]
...
# Daten in Training und Test aufteilen (z. B. 70% Training, 30% Test)
set.seed(123)
trainIndex <- createDataPartition(svm_data$premium_category, p = 0.7, list = FALSE)
...
# Konfusionsmatrix zur Bewertung der Modellleistung
conf_matrix <- confusionMatrix(predictions, test_data$premium_category)
print(conf_matrix)

# Genauigkeit der Trainingsdaten
pred_train <- predict(svm_model, train_data)
print(confusionMatrix(pred_train, train_data$premium_category))

# Genauigkeit der Testdaten
pred_test <- predict(svm_model, test_data)
print(confusionMatrix(pred_test, test_data$premium_category))
...
# Berechnung des MCC für die Testdaten
mcc_test <- mccr(predictions, test_data$premium_category)
print(paste("MCC for Test Data:", round(mcc_test, 4)))
...
# Berechnung des MCC für die Trainingsdaten. 0 stimmt aber nicht meiner Meinung...
mcc_train <- mccr(pred_train, train_data$premium_category)
print(paste("MCC for Train Data:", round(mcc_train, 4)))

```


```{r SVM_full_percentiles, cache=TRUE}
# Parallelisierung einrichten
cl <- makeCluster(detectCores() - 1) # Verwende alle verfügbaren Kerne außer einem
registerDoParallel(cl)
...
percentiles_full <- quantile(clean_dat_motor$PREMIUM_log, probs = c(0.25, 0.5, 0.75))

# Assign categories based on the full dataset's percentiles
clean_dat_motor$premium_category <- cut(clean_dat_motor$PREMIUM_log,
                                         breaks = c(-Inf, percentiles_full[1], percentiles_full[2], percentiles_full[3], Inf),
                                         labels = c("low", "medium", "high", "very_high"))
...
This analysis explores the performance of a multiclass classification task using an SVM model with a radial kernel. The goal was to classify PREMIUM_log into categories: "low," "medium," "high," and "very_high," employing hyperparameter tuning and parallel computation for efficiency. Data preparation included sampling 1,000 entries from the cleaned dataset and visualizing the distribution of PREMIUM_log, followed by a 70/30 split for training and testing. It is important to note that the dataset is imbalanced, which may pose challenges in model training and evaluation, potentially impacting the reliability of certain performance metrics.

Model training was carried out with 10-fold cross-validation repeated three times, leveraging parallel processing to speed up the evaluation. A grid search was performed to find optimal values for the hyperparameters .C and .sigma, ensuring the model's robustness and generalizability.
...
The code process incorporated parallelized cross-validation and grid search, facilitating comprehensive hyperparameter tuning. The findings highlighted an overall accuracy of 0.7771 and a Kappa statistic of 0.7029, with McNemar's test yielding a significant P-value of 0.008264, suggesting a noteworthy difference from random classification.

To improve the model, checking the training set's class distribution and considering resampling techniques like SMOTE may be beneficial. Additionally, validating MCC calculations to ensure accuracy, especially for anomalous values, is recommended.
...
As previous results were already good but modifications were still possible, hyperparameter tuning will be used.
...
print(tune.out) # best performance: 0.2909148
...
predictions <- predict(fit.improved, test)
confusionMatrix(predictions, test$premium_category)


```

The best model was with cost = 10 and gamma of 0.5 but it did not seem to improve the model compared to previous one. To better capture variance in the dataset, cross-validation will be tried to check if it improves overall performance.

```{r SVM cross-validation, cache=TRUE}
...
# View the best parameters and summary
print(tune.out) # best performance: 0.2909148
best_model_cv <- tune.out$best.model
...
# Fit the best model on the train data
fit.improved_cv <- best_model_cv

summary(fit.improved_cv)

predictions <- predict(fit.improved_cv, test)
confusionMatrix(predictions, test$premium_category)
```
This cross-validated model exhibits marginal improvements in overall accuracy, Kappa, and balanced accuracy for "low," "medium," and "very_high" classes, indicating better generalization. This suggests that cross-validation helped find a model that performs more consistently across different subsets of the data.
Accuracy improved from 70.18% to 71.11% indicating that the model is slightly better at classifying instances in a correct manner.
LOW: minor improvement in sensitivity indicating better identifications of "low" classifications.
Medium: Also a minor improvement of about 0.015
High: Slightly worsened with the advantage of having increased specificity to avoid false positives.

Generally speaking, adapting the cost to 1 led to improved sensitivity for medium and very_high without drastically reducing specificity.
The gamma of 0.5 indicated that no excessive complexities were applied to correctly identify classes.

This model is an improvement over previous versions, with a slight increase in accuracy and balanced accuracy across most classes. The careful tuning of cost and gamma, combined with cross-validation, allowed for a more balanced classification without sacrificing performance on any particular class.
