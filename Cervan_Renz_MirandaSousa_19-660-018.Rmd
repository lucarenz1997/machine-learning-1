...
data_motor$MAKE <- toupper(data_motor$MAKE)
...
table(data_motor$MAKE)


#Manuelle Korrekturen von MAKE
data_motor$MAKE[data_motor$MAKE %in% c("PORCHE", "FORSCHE")] <- "PORSCHE"
data_motor$MAKE[data_motor$MAKE == "YAMHA"] <- "YAMAHA"
data_motor$MAKE[data_motor$MAKE %in% c("VOLKSWAGON", "VOLKS WAGON")] <- "VOLKSWAGEN"
data_motor$MAKE[data_motor$MAKE %in% c("TOYOTAA", "TOYOTA*", "TOYTA", "TOYOTA AUTOMOBILE",
                                       "TOYATA", "T0Y0TA", "COMPACT YARIS", "YARIS",
                                       "LAND CRUISER", "VITZ")] <- "TOYOTA"
data_motor$MAKE[grepl("^TOYOTA", data_motor$MAKE)] <- "TOYOTA"
data_motor$MAKE[data_motor$MAKE %in% c("NISAN", "NISSAN*")] <- "NISSAN"
data_motor$MAKE[grepl("^NISSAN", data_motor$MAKE)] <- "NISSAN"
data_motor$MAKE[data_motor$MAKE %in% c("RENGE  ROVER", "RANGEROVER")] <- "RANGE ROVER"
data_motor$MAKE[data_motor$MAKE %in% c("RENALT", "RENUALT", "RENAULT/STOLARCZYK", "RENAULT*")] <- "RENAULT"
data_motor$MAKE[data_motor$MAKE %in% c("PEUGEOUT", "PEJOT", "PAGOT")] <- "PEUGEOT"
data_motor$MAKE[grepl("^PEUGEOT", data_motor$MAKE)] <- "PEUGEOT"
data_motor$MAKE[data_motor$MAKE %in% c("BMB")] <- "BMW"
data_motor$MAKE[grepl("^BMW", data_motor$MAKE)] <- "BMW"
data_motor$MAKE[data_motor$MAKE %in% c("MISTIBUSH", "MITSUBISHI*", "MITSUBUSHI")] <- "MITSUBISHI"
data_motor$MAKE[grepl("^FORD", data_motor$MAKE)] <- "FORD"
data_motor$MAKE[data_motor$MAKE %in% c("SPORTAGE")] <- "KIA"
data_motor$MAKE[data_motor$MAKE %in% c("MERCEEDES", "MERCEEDICE", "MERCEDICE", "MERCHEDES")] <- "MERCEDES"
data_motor$MAKE[grepl("^MERCEDES", data_motor$MAKE)] <- "MERCEDES"
data_motor$MAKE[data_motor$MAKE %in% c("SUZIKE")] <- "SUZUKI"
data_motor$MAKE[grepl("^SUZUKI", data_motor$MAKE)] <- "SUZUKI"
data_motor$MAKE[data_motor$MAKE %in% c("HYUNDI GETZ")] <- "HYUNDAI"
data_motor$MAKE[data_motor$MAKE %in% c("ISUSU")] <- "ISUZU"
data_motor$MAKE[grepl("^ISUZU", data_motor$MAKE)] <- "ISUZU"
data_motor$MAKE[data_motor$MAKE %in% c("FRANKUN IVECO")] <- "IVECO"
data_motor$MAKE[grepl("^IVECO", data_motor$MAKE)] <- "IVECO"
data_motor$MAKE[data_motor$MAKE %in% c("LANDROVER")] <- "LAND ROVER"
data_motor$MAKE[data_motor$MAKE %in% c("DONGFANG", "DONFING", "DONG FENGSHEN", "DONG FENG")] <- "DONGFENG"
data_motor$MAKE[data_motor$MAKE %in% c("MAHANDRA")] <- "MAHINDRA"


data_motor$MAKE[data_motor$MAKE %in% c("GELYION", "GENLION", "GELION", "GENLYONIVECO", "HONGYAN")] <- "GENLYON"
data_motor$MAKE[data_motor$MAKE %in% c("DHATSU", "DIATSU", "DIAHATSU")] <- "DAIHATSU"
data_motor$MAKE[data_motor$MAKE %in% c("DAWOO", "DAWWO", "DEAWOO", "DEAWOO USE")] <- "DAEWOO"
data_motor$MAKE[grepl("^LIFAN", data_motor$MAKE)] <- "LIFAN"
data_motor$MAKE[grepl("^BAIC", data_motor$MAKE)] <- "BAIC"
data_motor$MAKE[grepl("^LOADER", data_motor$MAKE)] <- "LOADER"
data_motor$MAKE[data_motor$MAKE %in% c("MOTOR CYCLE", "MOTOR  CYCLE")] <- "MOTORCYCLE"
data_motor$MAKE[data_motor$MAKE %in% c("AUTO", "AUTOMOBIL")] <- "AUTOMOBILE"
data_motor$MAKE[grepl("^CATERPILLAR", data_motor$MAKE)] <- "CATERPILLAR"
data_motor$MAKE[data_motor$MAKE %in% c("LIBER DOZER", "LIBERR MOBILE CRANE",
...
# Definiere die gewünschten Marken
desired_makes <- c("TOYOTA", "ISUZU", "NISSAN", "IVECO", "MITSUBISHI",
                   "FORD", "HYUNDAI", "MAZDA", "MERCEDES", "FIAT",
                   "SUZUKI", "RENAULT")

# Erstellen eines neuen Datensatzes mit den gefilterten Marken
filtered_data_motor <- data_motor[data_motor$MAKE %in% desired_makes, ]

# Überprüfen der Anzahl der verbleibenden Datensätze
cat("Anzahl der verbleibenden Datensätze:", nrow(filtered_data_motor), "\n")


table(filtered_data_motor$CLAIM_PAID)
table(data_motor$CLAIM_PAID)
table(filtered_data_motor$TYPE_VEHICLE)
###############################################################################################
#CCM_TON
summary(data_motor$CCM_TON)
