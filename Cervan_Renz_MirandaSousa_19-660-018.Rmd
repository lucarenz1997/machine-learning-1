...
##Poisson: (Profile-Likelihood Confidence Intervals)
#confint(poisson_model)
##Poisson: (Wald Confidence Interval)
#coef(poisson_model)["F"]
#c(-1, 1) * qnorm(1 - 0.95 / 2) * summary(poisson_model)$coefficients["F", "Std. Error"]
##Poisson: Point Estimate, 95% Confidence Interval
#pred <- predict(herz.fit, se.fit = TRUE, type = "link")
#pe <- exp(pred$fit)
#lb <- exp(pred$fit + qnorm(1 - 0.05 / 2) * pred$se.fit) #lower bound
#ub <- exp(pred$fit - qnorm(1 - 0.05 / 2) * pred$se.fit) #upper bound
#rbind(lb, ub)

#Poisson: Inference
#Significance test for individual parameters
```
