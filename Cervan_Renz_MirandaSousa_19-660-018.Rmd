...
Zielvariable: Anzahl Schadensmeldungen nach 5 Jahren Vertragslaufzeit?
...
#Poisson Devianztest: Wie gut passt Modell zu den Daten?
#Berechne p-Wert fuer H0: Das Modell beschreibt die Daten adaequat:
1 - pchisq(deviance(poisson_model), df.residual(poisson_model))
#Goodness-of-Fit (Residuenanalyse nur machen wenn wir genuegend Anzl. Beobachtungen haben)
deviance(poisson_model) #D
## [1] 1040
df.residual(poisson_model) #n - (m + 1)
#[1] 47
#Falls D >> (n - (m + 1)), dann ist das Modell ungen?gend
#Poisson Devianztest: Wie gut passt Modell zu den Daten? Goodness-of-Fit
1 - pchisq(deviance(poisson_model), df.residual(poisson_model)) #N0 gr?sser 0.05 -> Modell OK
#0.25 > 0.05
#Die Nullhypothese (betrachtetes Modell beschreibt die Daten ad?quat)
#wird nicht verworfen. Dies ist ein Hinweis, dass keine Overdispersion
#vorliegt und man mit dem gegebenen Modell weiterfahren kann
...
rbind(lb, ub)
#Punktschaetzung Poisson
x0= data.frame(Time=28/6)
...
# ggplot2 Plot: pramie und anzahl meldungen
poisson_model$model$fitted <- predict(poisson_model, type = "response")

ggplot(poisson_model$model) + 
  geom_point(aes(PREMIUM, AMOUNT_CLAIMS)) +
  geom_line(aes(PREMIUM, fitted)) + 
  labs(x = "\n PREMIUM", y = "Number Of Claims \n", 
       title = "Poisson Regression: Comparing PREMIUM To Number Of Claims  \n") +
  theme(plot.title = element_text(hjust = 0.5),
...
```{r GAM, cache=TRUE}
# TODO
#Bsp. Additives Modell anpassen
#Daten vorbereiten
library(car)
library(gam)
data(Prestige)
#Additives Modell anpassen
fit <-gam(prestige ~lo(income) +lo(education), data= Prestige) 
plot(fit, se= TRUE, residuals= TRUE) #Partielle Residuenplots VI 95% Nicht lineare Effekte anschauen
#Gibt es einen Nichtlinearen Anteil? Siehe Anova for Parametric Effects
summary(fit)

#Vorhersage
#Bsp: Ansehen Einkommen bei 15J Ausbildung & Einkommen von 40000 Dollar?
Prestige$lincome <-log(Prestige$income) #Logarithmieren
fit2 <-gam(prestige ~lo(lincome) +lo(education), data = Prestige)
x0 <-data.frame(lincome= log(40000),education= 15)
predict(fit2, newdata= x0)
